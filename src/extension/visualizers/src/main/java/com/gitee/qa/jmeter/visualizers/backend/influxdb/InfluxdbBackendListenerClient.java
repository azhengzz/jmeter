...
package com.gitee.qa.jmeter.visualizers.backend.influxdb;
...
public class InfluxdbBackendListenerClient extends AbstractBackendListenerClient implements Runnable {
    ...
    private static final String TAG_RAW_RESPONSE_CODE = ",rawResponseCode=";
    private static final String TAG_RESPONSE_MESSAGE = ",responseMessage=";
    private static final String TAG_RESPONSE_BODY = ",responseBody=";
    private static final String TAG_REQUEST_HEADER = ",requestHeader=";
    private static final String TAG_SAMPLE_DATA = ",sampleData=";
    ...
    protected void sendMetrics() {
        ...
        synchronized (LOCK) {
            for (Map.Entry<String, SamplerMetric> entry : getMetricsInfluxdbPerSampler().entrySet()) {
                ...
                // 每次获取完Metric数据后，重置Metric为初始值，用于下次记录。
    ...
    private void addMetrics(String transaction, SamplerMetric metric) {
        ...
        metric.getErrors().forEach((error, count) -> addErrorMetric(transaction, error.getResponseCode(), error.getResponseCode(),
                error.getResponseMessage(), "", "", "", count));
    }

    private void addErrorMetric(String transaction, String responseCode, String rawResponseCode, String responseMessage, String responseBody, String requestHeader, String sampleData, long count) {
        if (count > 0) {
            ...
            tag.append(TAG_RAW_RESPONSE_CODE).append(AbstractInfluxdbMetricsSender.tagToStringValue(rawResponseCode));
//            tag.append(TAG_RESPONSE_MESSAGE).append(AbstractInfluxdbMetricsSender.tagToStringValue(responseMessage));
            // 对几个大字符串进行排序后截取，尽量保留更多的字段内容 start
            ...
            Map<String, String> sortedMap = new TreeMap<>(new Comparator<String>() {
                ...
                public int compare(String k1, String k2) {
                    return map.get(k1).length() - map.get(k2).length();
                }
            });
            sortedMap.putAll(map);
            Set<Map.Entry<String, String>> sortedMaps = sortedMap.entrySet();
            for (Map.Entry<String, String> m: sortedMaps) {
                if ("responseMessage".equals(m.getKey())){
                    tag.append(TAG_RESPONSE_MESSAGE).append(subStr(AbstractInfluxdbMetricsSender.tagToStringValue(responseMessage.replace('\n', ' '))));
                } else if ("responseBody".equals(m.getKey())){
                    tag.append(TAG_RESPONSE_BODY).append(subStr(AbstractInfluxdbMetricsSender.tagToStringValue(responseBody.replace('\n', ' '))));
                } else if ("requestHeader".equals(m.getKey())){
                    tag.append(TAG_REQUEST_HEADER).append(subStr(AbstractInfluxdbMetricsSender.tagToStringValue(requestHeader.replace('\n', ' '))));
                } else if ("sampleData".equals(m.getKey())){
                    tag.append(TAG_SAMPLE_DATA).append(subStr(AbstractInfluxdbMetricsSender.tagToStringValue(sampleData.replace('\n', ' '))));
                }
            }
            // influxdb超长数据截取
//            tag = new StringBuilder(subStr(tag.toString()));
            // 对几个大字符串进行排序后截取，尽量保留更多的字段内容 end
    ...
    /**
     * 保存 Sample 执行结果
     * sampleResults 来自于 BackendListener 后台线程从队列中获取 Sample 的执行结果
     * */
    @Override
    ...
    private String subStr(String str){
        // influxdb要求写入数据小于64KB
        // if (str.length() >= 1024 * 64) {
        //    return str.substring(0, 1024 * 60) + "...[内容过大已截取]";

        // victoria-metrics要求label数据小于4KB
        if (str.length() >= 1024 * 4) {
                return str.substring(0, 1024 * 4 - 100) + "...[内容过大已截取]";
        } else {
            return str;
        }
    }

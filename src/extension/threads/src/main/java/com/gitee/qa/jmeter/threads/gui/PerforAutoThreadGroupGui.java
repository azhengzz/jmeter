...
package com.gitee.qa.jmeter.threads.gui;
...
public class PerforAutoThreadGroupGui extends AbstractThreadGroupGui implements ItemListener {
    ...
    public PerforAutoThreadGroupGui() {
        this(true);
    }

    public PerforAutoThreadGroupGui(boolean showDelayedStart) {
        super();
    ...
    public TestElement createTestElement() {
        PerforAutoThreadGroup tg = new PerforAutoThreadGroup();
    ...
    public void modifyTestElement(TestElement tg) {
        ...
        tg.setProperty(PerforAutoThreadGroup.RAMP_TIME, rampInput.getText());
        if (showDelayedStart) {
            tg.setProperty(PerforAutoThreadGroup.DELAYED_START, delayedStart.isSelected(), false);
        }
        tg.setProperty(new BooleanProperty(PerforAutoThreadGroup.SCHEDULER, scheduler.isSelected()));
        tg.setProperty(PerforAutoThreadGroup.DURATION, duration.getText());
        tg.setProperty(PerforAutoThreadGroup.DELAY, delay.getText());

        tg.setProperty(PerforAutoThreadGroup.SCENARIO, lcScenario.getText());
        tg.setProperty(PerforAutoThreadGroup.RECORD_FILE_PATH, recordFilePath.getText());
        tg.setProperty(PerforAutoThreadGroup.STOP_DELAY, stopDelay.getText());
    ...
    public void configure(TestElement tg) {
        ...
        rampInput.setText(tg.getPropertyAsString(PerforAutoThreadGroup.RAMP_TIME));
        loopPanel.configure((TestElement) tg.getProperty(AbstractThreadGroup.MAIN_CONTROLLER).getObjectValue());
        if (showDelayedStart) {
            delayedStart.setSelected(tg.getPropertyAsBoolean(PerforAutoThreadGroup.DELAYED_START));
        }
        scheduler.setSelected(tg.getPropertyAsBoolean(PerforAutoThreadGroup.SCHEDULER));

        toggleSchedulerFields(scheduler.isSelected());

        duration.setText(tg.getPropertyAsString(PerforAutoThreadGroup.DURATION));
        delay.setText(tg.getPropertyAsString(PerforAutoThreadGroup.DELAY));

        lcScenario.setText(tg.getPropertyAsString(PerforAutoThreadGroup.SCENARIO));
        recordFilePath.setText(tg.getPropertyAsString(PerforAutoThreadGroup.RECORD_FILE_PATH));
        stopDelay.setText(tg.getPropertyAsString(PerforAutoThreadGroup.STOP_DELAY));
   ...
   private void init() { // WARNING: called from ctor so must not be overridden (i.e. must be private or final)
        ...
        intgrationPanel.add(createPTPanel());

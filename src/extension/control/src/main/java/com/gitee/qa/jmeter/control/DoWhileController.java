...
public class DoWhileController extends GenericController implements Serializable, IteratingController {
    private static final Logger log = LoggerFactory.getLogger(DoWhileController.class);

    private static final long serialVersionUID = 233L;

    private static final String CONDITION = "DoWhileController.condition"; // $NON-NLS-1$

    private boolean breakLoop;

    public DoWhileController() {
        super();
    }
    ...
    private boolean endOfLoop(boolean loopEnd) {
        ...
        // TODO 如果条件表达式解析错误则可能会导致陷入死循环，例如表达式 ${__javaScript(${not_exist_var} === 1,)}
        // TODO While循环控制器和DoWhile循环控制器均存在此问题

...
import com.gitee.qa.jmeter.control.LoopController;
 ...
 */
//@GUIMenuSortOrder(4)
public class LoopControlPanel extends LogicControllerGui implements ActionListener {
    private static final long serialVersionUID = 240L;

    /**
     * A checkbox allowing the user to specify whether or not
     * the controller
     * should loop forever.
     */
     ...
     * 该变量只在当前循环控制器内生效
    * */
    private JTextField loopCounterName;

    /*
    * 循环计数初始值
    * */
    private JTextField loopCounterStartNum;
    ...
    public void configure(TestElement element) {
        ...
        if (element instanceof LoopController) {
            ...
            if (loopCounterName != null){
                loopCounterName.setText(((LoopController) element).getLoopCounterName());
            }
            if (loopCounterStartNum != null){
                loopCounterStartNum.setText(((LoopController) element).getLoopCounterStartNum());
            }
        } else {
            setState(1);
        }
    ...
    public void modifyTestElement(TestElement lc) {
        ...
        if (lc instanceof LoopController) {
            ...
            if (loopCounterName != null && loopCounterName.getText().length() > 0) {
                ((LoopController) lc).setLoopCounterName(loopCounterName.getText());
            }
            if (loopCounterStartNum != null && loopCounterStartNum.getText().length() > 0) {
                ((LoopController) lc).setLoopCounterStartNum(loopCounterStartNum.getText());
            }
        }
    }
    ...
    public void clearGui() {
        ...
        loopCounterName.setText("");
        loopCounterStartNum.setText("1");
    }
    ...
    public String getLabelResource() {
        return "loop_controller_with_var_title"; // $NON-NLS-1$
    }

    @Override
    public String getStaticLabel(){
        return "循环控制器(带变量)"; // $NON-NLS-1$
    }
    ...
    private void init() { // WARNING: called from ctor so must not be overridden (i.e. must be private or final)
        ...
        if (displayName) {
            ...
            mainPanel.add(createLoopCounterNamePanel());
            mainPanel.add(createLoopCounterStartNumPanel());
     ...
     * 循环计数变量名
     * */
    private JPanel createLoopCounterNamePanel() {
        JPanel loopCounterNamePanel = new HorizontalPanel();
//        JLabel counterNameLabel = new JLabel(JMeterUtils.getResString("loop_counter_variable_name")); // $NON-NLS-1$
        JLabel counterNameLabel = new JLabel("循环计数变量名"); // $NON-NLS-1$
        loopCounterNamePanel.add(counterNameLabel);
        loopCounterName = new JTextField("");
        counterNameLabel.setLabelFor(loopCounterName);
        loopCounterNamePanel.add(loopCounterName);

        return loopCounterNamePanel;
    }

    /**
     * 循环计数初始值
     * */
    private JPanel createLoopCounterStartNumPanel() {
        JPanel panel = new HorizontalPanel();
        JLabel label = new JLabel("循环计数初始数值"); // $NON-NLS-1$
        panel.add(label);
        loopCounterStartNum = new JTextField("");
        label.setLabelFor(loopCounterStartNum);
        panel.add(loopCounterStartNum);
        return panel;
    }

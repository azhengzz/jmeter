...
public class LoopController extends GenericController implements Serializable, IteratingController, LoopIterationListener {
    ...
    private static final String LOOP_COUNTER_NAME = "LoopController.loop_counter_name"; // $NON-NLS-1$

    private static final String LOOP_COUNTER_START_NUM = "LoopController.loop_counter_start_num"; // $NON-NLS-1$
    ...
    public void setLoopCounterName(String value){
        setProperty(LOOP_COUNTER_NAME, value);
    }

    public String getLoopCounterName() {
        return getPropertyAsString(LOOP_COUNTER_NAME);
    }

    public void setLoopCounterStartNum(String value) {
        setProperty(LOOP_COUNTER_START_NUM, value);
    }

    // 获取循环计数开始数值
    public String getLoopCounterStartNum() {
        return getPropertyAsString(LOOP_COUNTER_START_NUM, "1");
    }
     ...
     * 在循环控制内部更新计数器
     */
    private void updateLoopCounterValue(){
        JMeterVariables variables = JMeterContextService.getContext().getVariables();
        if(variables != null) {
            variables.put(getLoopCounterName(), String.valueOf(loopCount + Integer.parseInt(getLoopCounterStartNum())));
        }
    }
     ...
     * 循环结束后清理计数器
     * */
    private void delLoopCounterVariable(){
        JMeterVariables variables = JMeterContextService.getContext().getVariables();
        if(variables != null) {
            variables.remove(getLoopCounterName());
        }
    }
